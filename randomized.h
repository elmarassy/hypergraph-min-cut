...
public:
    explicit DSU(uint32_t n) : parent(n), rank(n, 0), num_components(n) {
...
double redoProbability(uint32_t n, uint32_t edgeSize, uint32_t k) noexcept {
    if (k <= 1) return 0.0;
    if (edgeSize >= n) return 1.0;
    if (n - edgeSize < k - 1) return 1.0;
    double logProd = 0.0;
    for (uint32_t i = 0; i <= k - 2; ++i) {
        logProd += std::log(static_cast<double>(n - edgeSize - i))
...
size_t chooseRandomEdge(const Hypergraph& H, std::mt19937_64& rng) {
    if (H.edges.empty()) return SIZE_MAX;
    ...
    return static_cast<size_t>(
            std::lower_bound(cumulative.begin(), cumulative.end(), target + 1) - cumulative.begin()
    );
}
...
ContractionResult randomizedMinKCut(
        const Hypergraph& H,
        uint32_t k,
        uint64_t batchScale,
        uint64_t numBatchesFactor,
        uint64_t baseSeed
) {
    ...
    for (uint64_t batch = 0; batch < numBatches; batch++) {
        ...
        for (uint64_t iter = 0; iter < iterationsPerBatch; ++iter) {
            try {
                ...
                std::cout << "Batch #" << batch << " timed out at iteration "
                          << iter << "/" << iterationsPerBatch << "\n";
                break;
            }
        }
        ...
        if (anySuccess) {
            ...
            std::cout << "Batch #" << batch << " completed in " << batchRuntime
                      << " units (" << (100.0 * batchRuntime / cutoff) << "% of cutoff)\n";
        } else {
    ...
    auto best = std::min_element(batchBests.begin(), batchBests.end(),
                                 [](const auto& a, const auto& b) { return a.cutWeight < b.cutWeight; });

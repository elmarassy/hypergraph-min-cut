...
int main() {
    uint64_t seed = 2;
    auto h = kUniformHypergraph(std::uniform_int_distribution<uint32_t>(1, 1), 1000, 1000, 5, seed);

    auto randomizedStart = std::chrono::high_resolution_clock::now();
    auto randomizedCut = randomizedMinKCut(h, 2, 2, 2, seed);
    auto randomizedEnd = std::chrono::high_resolution_clock::now();

    auto deterministicStart = std::chrono::high_resolution_clock::now();
    auto deterministicCut = deterministicMinKCut(h, 2);
    auto deterministicEnd = std::chrono::high_resolution_clock::now();

    std::cout << "Randomized runtime: " << std::chrono::duration_cast<std::chrono::milliseconds>(randomizedEnd - randomizedStart) << std::endl;
    std::cout << "Deterministic runtime: " << std::chrono::duration_cast<std::chrono::milliseconds>(deterministicEnd - deterministicStart) << std::endl;

    std::cout << "Randomized cut value: " << randomizedCut.cutWeight << std::endl;
    std::cout << "Deterministic cut value: " << deterministicCut.cutWeight << std::endl;
}